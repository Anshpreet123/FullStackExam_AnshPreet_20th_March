...
const CartPage = () => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      ...
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/cart`, { headers });
  ...
  const handleRemove = async (productId: string) => {
    ...
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/cart/remove/${productId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleQuantityChange = async (productId: string, quantity: number) => {
    ...
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/cart/add`, { productId, quantity }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleCheckout = async () => {
    ...
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/orders/checkout`, { items: cart }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
